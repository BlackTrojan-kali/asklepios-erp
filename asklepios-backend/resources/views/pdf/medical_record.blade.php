...
<head>
    ...
    <style>
        ...
        .watermark { position: fixed; top: 25%; left: 15%; width: 70%; text-align: center; opacity: 0.12; z-index: -1000; }
        .watermark img { width: 100%; max-width: 500px; }
        header { position: fixed; top: -110px; left: 0; right: 0; height: 90px; border-bottom: 3px solid #003366; padding-bottom: 10px; }
        .header-table { width: 100%; border-collapse: collapse; }
        .header-table td { vertical-align: middle; }
        .hospital-logo { max-height: 70px; max-width: 180px; }
        .doc-title-container { text-align: right; }
        .doc-title { margin: 0; font-size: 22px; color: #003366; text-transform: uppercase; letter-spacing: 2px; font-weight: 900; }
        .doc-subtitle { margin: 2px 0 0 0; font-size: 12px; font-weight: bold; color: #00a896; }
        .hospital-info { font-size: 10px; color: #475569; margin-top: 5px; }
        .hospital-name { font-size: 18px; font-weight: bold; color: #003366; text-transform: uppercase; }
        footer { position: fixed; bottom: -40px; left: 0; right: 0; height: 30px; border-top: 2px solid #00a896; text-align: center; font-size: 9px; color: #64748b; padding-top: 5px; }
        .page-number:after { content: counter(page); }
        .section { margin-bottom: 25px; }
        .section-title { background-color: #003366; color: #ffffff; padding: 6px 12px; font-size: 13px; font-weight: bold; text-transform: uppercase; border-left: 6px solid #00a896; margin-bottom: 12px; }
        .data-table { width: 100%; border-collapse: collapse; margin-bottom: 15px; }
        .data-table th, .data-table td { border: 1px solid #cbd5e1; padding: 6px 8px; vertical-align: top; }
        .data-table th { background-color: #f0fdfa; color: #003366; font-weight: bold; width: 20%; text-align: left; }
        .data-table td { background-color: #ffffff; color: #334155; }
        .timeline-item { border: 1px solid #e2e8f0; border-left: 4px solid #003366; border-radius: 3px; padding: 10px; margin-bottom: 15px; background-color: #ffffff; page-break-inside: avoid; }
        .timeline-header { font-weight: bold; color: #003366; border-bottom: 1px dashed #cbd5e1; padding-bottom: 5px; margin-bottom: 8px; font-size: 12px; }
        .sub-title { color: #00a896; font-size: 11px; font-weight: bold; margin-top: 10px; margin-bottom: 5px; text-transform: uppercase; }
        .badge { display: inline-block; background: #e2e8f0; padding: 2px 6px; border-radius: 3px; font-size: 10px; font-weight: bold; margin-right: 4px; margin-bottom: 4px; border: 1px solid #cbd5e1; }
...
<body>
            ...
            <div class="timeline-item">
                ...
                @if(isset($admission->performedMedicalActs) && $admission->performedMedicalActs->count() > 0)
                    ...
                    <ul style="margin: 5px 0; padding-left: 20px;">
                        @foreach($admission->performedMedicalActs as $act)
                ...
                @if(isset($admission->consultations) && $admission->consultations->count() > 0)
                    ...
                    @foreach($admission->consultations as $consult)
                        <div style="margin-left: 10px; margin-bottom: 10px; border-left: 2px solid #cbd5e1; padding-left: 10px;">
                            <p class="mb-2">
                                <strong>Date :</strong> {{ \Carbon\Carbon::parse($consult->created_at)->format('d/m/Y à H:i') }} |
                                <strong>Vu par :</strong> Dr. {{ $consult->profileDoctor->user->first_name ?? 'Inconnu' }} {{ $consult->profileDoctor->user->last_name ?? '' }} 
                            </p>
                            
                            <p class="mb-2"><strong>Motif / Évolution :</strong> {{ $consult->chief_complaint }}</p>
    ...
    <div class="section">
        <div class="section-title">4. Historique des Consultations Externes</div>
