...
<head>
    ...
    <style>
        ...
        .exam { margin-bottom: 10px; font-size: 16px; }
...
<body>
    ...
    <div class="info-section">
        <div class="doctor-info">
            <strong>Prescripteur : Dr. {{ $doctor->user->first_name }} {{ $doctor->user->last_name }}</strong><br>
            {{ $doctor->specialty }}<br>
        </div>
        <div class="patient-info">
            ...
            Sexe : {{ $patient->gender }} | Âge : {{ \Carbon\Carbon::parse($patient->date_of_birth)->age }} ans<br>
        </div>
        <div class="clear"></div>
    </div>

    <div class="title">DEMANDE D'EXAMENS COMPLÉMENTAIRES</div>

    <div class="content">
        <p>Prière de bien vouloir pratiquer les examens suivants :</p>
        <ul>
            @foreach($examRequest->examRequestLines as $line)
                <li class="exam">{{ $line->exam_name }}</li>
            @endforeach
        </ul>
    </div>
