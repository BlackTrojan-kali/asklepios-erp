...
import { Building, DollarSign, Inbox, X, Check } from "lucide-react";
    ...
    <tbody className="divide-y divide-slate-100 dark:divide-gray-700 animate-pulse">
      {[1, 2, 3, 4, 5].map((i) => (
        <tr key={i} className="bg-white dark:bg-gray-800">
          <td className="p-4">
            <div className="h-4 bg-slate-200 dark:bg-gray-700 rounded w-24" />
          </td>
          <td className="p-4">
          ...
          <td className="p-4">
            <div className="h-4 bg-slate-200 dark:bg-gray-700 rounded w-16 text-right" />
          </td>
          <td className="p-4">
            <div className="h-8 bg-slate-200 dark:bg-gray-700 rounded w-28 mx-auto" />
          </td>
        </tr>
      ))}
    </tbody>
...
export default function AdminPendingDeposits() {
  ...
  const [showConfirmModal, setShowConfirmModal] =
    useState<PaymentTransactionDto | null>(null);
  ...
  return (
    <div className="p-6 bg-slate-50 dark:bg-gray-900 min-h-screen text-slate-800 dark:text-gray-200">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
            <DollarSign className="w-8 h-8 text-emerald-600 dark:text-emerald-400" /> Versements en
            Attente {selectedBranchName && ` - ${selectedBranchName}`}
          </h1>
        ...
        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 px-3 py-1.5 rounded-xl shadow-xs border border-slate-200 dark:border-gray-700">
          <Building className="w-4 h-4 text-slate-400 dark:text-gray-400" />
          <select
            ...
            className="bg-transparent border-none text-sm font-semibold text-slate-700 dark:text-gray-200 focus:outline-hidden cursor-pointer"
          >
            <option value="" className="dark:bg-gray-800 text-slate-800 dark:text-white">
              Toutes les pharmacies (Vue globale)
            </option>
            {branches.map((b) => (
              <option key={b.id} value={b.id} className="dark:bg-gray-800 text-slate-800 dark:text-white">
                {b.name}
              </option>
            ))}
          </select>
      ...
      <div>
        {loadingPending ? (
          <div className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-2xl overflow-hidden shadow-xs">
            <table className="w-full text-left text-sm">
              <TableSkeleton />
            </table>
          </div>
        ) : pendingTxList.length === 0 ? (
          <div className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-2xl p-12 text-center shadow-xs">
            <Inbox className="w-12 h-12 stroke-1 mx-auto mb-2 text-slate-400 dark:text-gray-500" />
            <p className="text-sm font-semibold text-slate-600 dark:text-gray-300">
              Aucun versement en attente à confirmer.
            </p>
          </div>
        ) : (
          <div className="bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 rounded-2xl overflow-hidden shadow-xs">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="bg-slate-50 dark:bg-gray-900/60 border-b border-slate-200 dark:border-gray-700 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-gray-400">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-gray-700">
                  {pendingTxList.map((tx) => (
                    <tr
                      key={tx.id}
                      className="hover:bg-slate-50/50 dark:hover:bg-gray-700/40 transition-colors"
                    >
                      ...
                      <td className="p-4">
                        {tx.session?.register?.name ? (
                          <span className="font-mono text-xs bg-slate-100 dark:bg-gray-700 text-slate-700 dark:text-gray-200 px-2 py-0.5 rounded border border-slate-200 dark:border-gray-600">
                          ...
                          <span className="text-slate-400 dark:text-gray-500 italic">Inconnu</span>
                        )}
                      </td>
                      <td className="p-4 font-semibold text-slate-700 dark:text-gray-300">
                        {tx.destination_account?.name || "Non affecté"}
                      </td>
                      <td className="p-4 text-right font-bold font-mono text-slate-900 dark:text-white text-base">
                        {tx.amount.toLocaleString()}{" "}
                        <span className="text-xs text-slate-500 dark:text-gray-400">XAF</span>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 border border-rose-300 dark:border-rose-800/60 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center gap-1"
                          >
