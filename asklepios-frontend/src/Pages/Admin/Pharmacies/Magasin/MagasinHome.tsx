...
const MagasinHome = () => {
  // Hooks des stores
  const { stocks, loading, getMyBranchStocks } = useStockStore();
  console.log("stocks", stocks);

  const { articles, getArticles } = useArticleStore();

  // États pour les filtres (Pas besoin du filtre succursale ici !)
  ...
  useEffect(() => {
    getMyBranchStocks({});
    getArticles({});
  }, [getMyBranchStocks, getArticles]);

  // Fonction de rafraîchissement manuel
  const handleRefresh = () => {
    getMyBranchStocks(filters);
    getArticles({});
  };
  ...
  const getExpirationStatus = (dateString?: string | null) => {
    if (!dateString) {
      return {
        ...
        color:
          "text-blue-600 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-800/50",
    ...
    if (expireDate < today) {
      return {
        label: "Périmé",
        color:
          "text-red-600 bg-red-100 dark:bg-red-900/30 dark:text-red-400 border-red-200 dark:border-red-800/50",
      ...
      return {
        ...
        color:
          "text-orange-600 bg-orange-100 dark:bg-orange-900/30 dark:text-orange-400 border-orange-200 dark:border-orange-800/50",
      ...
      return {
        label: "Valide",
        color:
          "text-emerald-600 bg-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800/50",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white">
              Inventaire de ma succursale
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Consultez et gérez les niveaux de stock de votre point de vente.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-2 bg-white hover:bg-gray-50 text-slate-700 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg font-medium transition-colors shadow-sm border border-gray-200 dark:border-gray-700 disabled:opacity-70"
        >
        ...
        >
          <div className="md:col-span-2">
            <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
              Recherche (Article, Lot, Code-barres)
            </label>
          ...
          <div className="md:col-span-1">
            ...
            <Select
              ...
              value={
                articleOptions.find(
                  (opt) => opt.value === filters.article_id,
                ) || null
              }
              onChange={(selected) =>
                setFilters({
      ...
      {/* TABLEAU DES STOCKS (Sans la colonne succursale) */}
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="p-12 text-center">
                    <div className="flex flex-col items-center justify-center text-gray-400 dark:text-gray-500">
                      ...
                      <p>
                        Aucun stock trouvé dans votre succursale pour ces
                        critères.
                      </p>
                    </div>
                  </td>
                ...
                stocks.map((stock) => {
                  ...
                  const expStatus = getExpirationStatus(
                    stock.batch?.expire_date,
                  );
                  const qtyStatus = getStockLevelStatus(
                    stock.qty,
                    ...
                    >
                      ...
                      <td className="p-4 align-top w-2/5">
                        <div className="flex flex-col">
                          <div className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-gray-200">
                            <Package size={16} className="text-[#00a896]" />
                      ...
                      <td className="p-4 align-top w-2/5">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <div
                            className={`inline-flex items-center gap-1 w-max px-2 py-0.5 rounded text-[11px] font-medium border ${expStatus.color}`}
                          >
                            {expStatus.icon}
                            {expStatus.label}
                          </div>
                      ...
                      <td className="p-4 text-right align-top w-1/5">
                        <div className="flex flex-col items-end gap-1.5">
                          ...
                          <div
                            className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-bold ${qtyStatus.color}`}
                          >
                            {qtyStatus.icon}
                            {qtyStatus.label}
                          </div>
