  ...
  useEffect(() => {
    if (token) {
      if (profile?.role == "super_admin") {
        ...
        if (
          roles.includes("lab_manager") ||
          roles.includes("lab_technician") ||
          roles.includes("lab_biologist")
        ) {
          navigate("/laboratory/sampling", { replace: true });
        } else if (roles.includes("lab_receptionist")) {
          navigate("/laboratory/patients", { replace: true });
        } else {
          navigate("/laboratory/sampling", { replace: true }); // Fallback
        }
      }
    }
  }, [token, navigate]);
