...
const StockTransfers = () => {
    ...
    const handleRefresh = () => {
        getTransfers({ 
            page, status: statusFilter, start_date: startDate, end_date: endDate 
        }, 'pharmacy');
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Transferts de Stock</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Gérez les expéditions et réceptions inter-pharmacies.</p>
                    </div>
                </div>
                
                <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
                    <button 
                        onClick={() => exportPdf({ status: statusFilter, start_date: startDate, end_date: endDate }, 'pharmacy')} 
            ...
            <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row gap-4 items-end">
                ...
                <div className="w-full md:flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Du (Date de création)</label>
                    ...
                    <div>
                        <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Au (Date de création)</label>
                        <div className="relative">
                            <Calendar className="absolute left-3 top-2.5 text-gray-400" size={16} />
                            <input 
                                type="date" 
                                        ...
                                        <tr className={`hover:bg-slate-50 dark:hover:bg-gray-800/50 transition-colors ${expandedTransferId === transfer.id ? 'bg-slate-50 dark:bg-gray-800/50' : ''}`}>
                                            ...
                                            <td className="p-4 text-right">
                                                <div className="flex justify-end items-center gap-2">
                                                    ...
                                                    >
                                                        <Eye size={14} /> {expandedTransferId === transfer.id ? 'Masquer' : 'Voir'}
                                                    </button>

                                                    {/* 2. BOUTON RECEVOIR (Condition: En transit + Je suis le destinataire) */}
                                                    {transfer.status === 'INITIATED' && transfer.destination_pharmacy_id === currentBranchId && (
                                                        <button 
                                                            onClick={() => handleReceive(transfer.id, transfer.sourcePharmacy?.name || '')}
                                                    ...
                                                    {transfer.status === 'INITIATED' && transfer.source_pharmacy_id === currentBranchId && (
                                                        <button 
                                                ...
                                                <td colSpan={6} className="p-0">
                                                    <div className="p-6">
                                                        <h4 className="text-xs font-bold text-indigo-800 dark:text-indigo-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                                                            <Package size={16} /> Contenu de l'expédition ({transfer.lines?.length || 0} références)
                                                        </h4>
                                                            ...
                                                            <table className="w-full text-left text-sm">
                                                                <thead className="bg-slate-50 dark:bg-gray-900/50 text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                                                                    <tr>
                                                                        ...
                                                                        <th className="px-4 py-2 font-semibold text-right">Qté Expédiée</th>
                                                                    </tr>
                                                                </thead>
