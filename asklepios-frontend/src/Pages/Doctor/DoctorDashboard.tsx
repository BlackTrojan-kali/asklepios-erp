...
const DoctorDashboard = () => {
  ...
  const [previewConsultationId, setPreviewConsultationId] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    if (selectedAppointment) {
      const freshAppt = appointments.find(
        (a) => a.id === selectedAppointment.id,
      );
      if (
        freshAppt &&
        JSON.stringify(freshAppt) !== JSON.stringify(selectedAppointment)
      ) {
  ...
  return (
    <div className="space-y-6 min-h-[85vh] flex flex-col relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm shrink-0">
        <div>
          <h1 className="text-2xl font-black text-[#003366] dark:text-blue-400 font-brand">
            Bonjour, Dr. {doctorName}
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 font-lato">
            Espace de régulation clinique et de suivi de la file d'attente.
          </p>
        </div>
        <button
        ...
        >
          <RefreshCw
            size={16}
            className={appointmentsLoading ? "animate-spin text-[#00a896]" : ""}
          />
          Actualiser la file
        </button>
      </div>
        ...
        <div className="w-full lg:w-5/12 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 bg-[#003366] text-white flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <Users size={18} className="text-[#00a896]" />{" "}
              <h2 className="font-bold text-sm uppercase tracking-wider font-brand">
                Patients Assignés
              </h2>
            </div>
            <span className="bg-[#00a896] text-white text-xs font-black px-2.5 py-0.5 rounded-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50 dark:bg-gray-800/30 custom-scrollbar">
            {appointmentsLoading ? (
              <div className="flex flex-col items-center justify-center py-20">
                <Loader2
                  size={36}
                  className="animate-spin text-[#00a896] mb-2"
                />
              </div>
            ) : sortedAppointments.length === 0 ? (
              <div className="text-center py-20 text-gray-400 dark:text-gray-500">
                <CalendarDays size={48} className="mx-auto opacity-30" />
              ...
              sortedAppointments.map((appt) => {
                ...
                const isWaiting =
                  appt.status === "ARRIVED" &&
                  appt.visit?.status === "IN_WAITING_ROOM";
                const isConsulting =
                  appt.status === "ARRIVED" &&
                  appt.visit?.status === "IN_CONSULTATION";
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-xl border transition-all cursor-pointer flex items-center justify-between group ${isSelected ? "bg-[#faf8f1] dark:bg-gray-700 border-[#00a896] shadow-sm" : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600"}`}
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      <div
                        className={`shrink-0 flex items-center justify-center font-black text-lg min-w-[3rem] h-12 rounded-xl ${isConsulting ? "bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400" : isWaiting ? "bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400" : isComplete ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400"}`}
                      >
                        <User size={20} />
                      </div>
                      <div className="min-w-0 space-y-0.5">
                        <h3 className="font-bold text-sm text-slate-800 dark:text-white truncate font-brand">
                          {appt.patient?.first_name} {appt.patient?.last_name}
                        </h3>
                        <p className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 font-mono">
                          <Clock size={12} />{" "}
                          {isScheduled
                            ? `RDV: ${new Date(appt.scheduled_datetime).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}`
                            : `Arrivée: ${appt.visit?.arrival_time ? new Date(appt.visit.arrival_time).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }) : "..."}`}
                        </p>

                        {/* 👉 AFFICHAGE DES BADGES (Ticket + Statut) */}
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          {appt.visit?.queue_number &&
                    ...
                    <ChevronRight
                      size={16}
                      className="text-gray-400 dark:text-gray-500 group-hover:translate-x-1 transition-transform"
                    />
                  </div>
                );
              })
        ...
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col overflow-hidden">
          {selectedAppointment ? (
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="p-6 border-b border-gray-100 dark:border-gray-700 bg-[#faf8f1]/50 dark:bg-gray-900/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
                <div className="flex items-center gap-4">
                  <div className="h-14 w-14 rounded-full bg-[#00a896]/10 text-[#00a896] flex items-center justify-center shrink-0">
                    <User size={28} />
                  </div>
                  <div>
                    <h2 className="text-xl font-bold text-slate-800 dark:text-white font-brand leading-tight">
                      {selectedAppointment.patient?.first_name}{" "}
                      {selectedAppointment.patient?.last_name}
                    </h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400 font-mono mt-0.5">
                      Code Patient : {selectedAppointment.patient?.patient_code}
                    </p>
                  </div>
                </div>
                      ...
                      <>
                        ...
                        <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700/50 px-3 py-1.5 rounded-lg border border-gray-100 dark:border-gray-600">
                          ...
                          <span className="font-bold text-slate-700 dark:text-gray-300 text-sm">
                            {medicalBg.allergies?.length
                              ? medicalBg.allergies.join(", ")
                              : "Aucune"}
                          </span>
                        </div>
                        <div className="w-full text-xs text-gray-600 dark:text-gray-400 mt-1">
                          <strong>Maladies chroniques : </strong>
                          {medicalBg.chronic_conditions?.length
                            ? medicalBg.chronic_conditions.join(", ")
                            : "Néant"}
                        </div>
                      </>
                    ) : (
                      <p className="text-sm text-gray-500 italic w-full">
                        Aucun dossier médical initialisé pour ce patient.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  {historyLoading ? (
                    <div className="flex justify-center py-10">
                      <Loader2
                        size={20}
                        className="animate-spin text-[#003366] dark:text-blue-400"
                      />
                    </div>
                  ) : consultations.length === 0 ? (
                    <div className="text-center py-12 bg-slate-50 dark:bg-gray-900/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
                      <FileText
                        size={40}
                      ...
                      {consultations.map((consult) => (
                        ...
                        >
                          <div className="flex justify-between items-center text-xs font-mono border-b border-gray-50 dark:border-gray-700 pb-1.5 text-gray-400 dark:text-gray-500">
                            <span className="group-hover:text-[#00a896] transition-colors font-bold">
                              Consulter les détails #{consult.id} &rarr;
                            </span>
                            <span>
                      ...
                      {historyPagination && historyPagination.lastPage > 1 && (
                        <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                          ...
                          <button
                            onClick={() =>
                              setHistoryPage((p) =>
                                Math.min(historyPagination.lastPage, p + 1),
                              )
                            }
                            disabled={
                              historyPage === historyPagination.lastPage ||
                              historyLoading
                            }
                            className="px-4 py-2 text-sm bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg disabled:opacity-50 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors font-medium"
                          >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-gray-400 dark:text-gray-500">
              <Activity
                size={56}
                className="text-gray-200 dark:text-gray-700 mb-3 animate-pulse"
              />
              <p className="text-base font-bold text-slate-700 dark:text-gray-300 font-brand">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-gray-100 dark:border-gray-800">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 font-lato">
              Êtes-vous sûr de vouloir supprimer cette consultation ? Si elle
              n'est pas encore facturée, le patient retournera en salle
              d'examen.
            </p>
