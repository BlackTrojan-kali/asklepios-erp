...
export const ConsultationModal: React.FC<ConsultationModalProps> = ({
  ...
  const handleSubmitConsultation = async () => {
    ...
    const payload: CreateConsultationPayload | any = {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="w-full lg:w-[480px] xl:w-[520px] border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800/50 flex flex-col overflow-y-auto custom-scrollbar shrink-0">
            ...
            <div className="flex border-b border-gray-200 dark:border-gray-700 bg-slate-100 dark:bg-gray-800/80 p-1 gap-1">
              <button onClick={() => setLeftTab("BG")} className={`flex-1 py-2 px-3 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${leftTab === "BG" ? "bg-white dark:bg-gray-900 text-[#003366] dark:text-blue-400 shadow-sm border border-gray-200 dark:border-gray-700" : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200"}`}>
                <Activity size={14} className="text-[#00a896]" /> Dossier Médical
              </button>
              <button onClick={() => setLeftTab("LAB")} className={`flex-1 py-2 px-3 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${leftTab === "LAB" ? "bg-white dark:bg-gray-900 text-[#003366] dark:text-blue-400 shadow-sm border border-gray-200 dark:border-gray-700" : "text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200"}`}>
                <TestTube size={14} className="text-purple-500" /> Analyses & Résultats
              </button>
            </div>
            ...
            {leftTab === "BG" ? (
              <div className="p-5 flex-1 space-y-5 text-sm">
                <div className="flex items-center justify-between mb-2 border-b border-gray-100 dark:border-gray-700 pb-2">
                  <h4 className="flex items-center gap-2 font-bold text-gray-700 dark:text-gray-300 font-brand uppercase tracking-wider text-xs">
                    <Activity size={14} className="text-[#00a896]" /> Antécédents Médicaux
                  </h4>
                  ...
                  <div className="space-y-5 text-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 p-2 rounded border border-gray-100 dark:border-gray-700">{localMedicalBg.lifestyle_habits || "Non renseigné"}</p>
                      </div>
                      <div>
                        ...
                        <p className="text-xs text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 p-2 rounded border border-gray-100 dark:border-gray-700">{localMedicalBg.family_history || "Non renseigné"}</p>
                      </div>
                    </div>
                    ...
                    {localMedicalBg.immunizations?.length > 0 && (
                      <div>
                        ...
                        <p className="text-xs text-gray-600 dark:text-gray-400 font-medium">{localMedicalBg.immunizations.join(" • ")}</p>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="p-4 flex-1 overflow-y-auto custom-scrollbar">
                {patientId ? (
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 italic text-center py-10">Patient non identifié.</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex-1 flex flex-col bg-[#faf8f1] dark:bg-gray-900">
            <div className="flex-1 p-6 overflow-y-auto space-y-6 custom-scrollbar">
              ...
              <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={chiefComplaint} onChange={(e) => setChiefComplaint(e.target.value)} className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#00a896] outline-none dark:text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">Notes de consultation</label>
                    <textarea value={clinicalNotes} onChange={(e) => setClinicalNotes(e.target.value)} rows={4} className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#00a896] outline-none resize-y dark:text-white" />
                  </div>
                </div>
              </div>
                    ...
                    {performedActs.map((act, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-gray-800 dark:text-gray-200">{sharedMedicalActs.find((a) => a.id === act.medical_act_catalog_id)?.name || "Acte inconnu"}</p>
                    ...
                    {prescriptions.map((med, idx) => (
                      <li key={idx} className="flex justify-between p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="font-bold text-sm text-gray-800 dark:text-gray-200">{med.custom_medication_name || allArticles.find((a) => a.id === med.article_id)?.name || `Article #${med.article_id}`}</p>
              ...
              <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                {exams.length === 0 ? (
                  <p className="text-sm text-gray-400 dark:text-gray-500 italic text-center py-2">Aucun examen prescrit pour cette séance.</p>
                ) : (
                  <div className="flex flex-wrap gap-2.5">
                    {exams.map((exam, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 px-3 py-2 rounded-xl border border-purple-200 dark:border-purple-800 text-xs font-bold shadow-xs">
                        <TestTube size={15} className="text-purple-500" />
                        <span>{exam.exam_name}</span>
                        <button onClick={() => removeExam(idx)} title="Retirer cet examen" className="text-purple-400 hover:text-red-500 dark:hover:text-red-400 transition-colors ml-1 p-0.5 rounded-md"><X size={14} /></button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                {transfusions.length === 0 ? (
                  <p className="text-sm text-gray-400 dark:text-gray-500 italic text-center py-2">
                    Aucune transfusion prescrite pour cette séance.
                  </p>
                ) : (
                  <ul className="space-y-2">
                    {transfusions.map((t, idx) => (
                      <li key={idx} className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div className="flex items-start gap-3">
                          <Droplet size={18} className="text-red-500 mt-0.5" />
                          <div>
                            <p className="font-bold text-sm text-gray-800 dark:text-gray-200">
                              Poche de Sang {t.blood_bag?.type === 'WHOLE_BLOOD' ? 'Total' : t.blood_bag?.type === 'RED_CELLS' ? 'Globules Rouges' : 'Plasma'} 
                              <span className="text-red-600 ml-1">(Gr. {t.blood_bag?.blood_type})</span>
                            </p>
                            <p className="text-xs text-gray-500 mt-0.5">
                              Admin prévue le : {new Date(t.start_time).toLocaleString('fr-FR')} • Code: {t.blood_bag?.barcode || `ID-${t.blood_bag_id}`}
                            </p>
                          </div>
                        </div>
                        <button onClick={() => removeTransfusion(idx)} className="text-red-400 hover:text-red-600">
                          <X size={16} />
                        </button>
                      </li>
                    ))}
        ...
        {successConsultationId && (
          <div className="absolute inset-0 z-[55] bg-[#faf8f1] dark:bg-gray-900 flex flex-col items-center justify-center p-6 animate-fadeIn">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-xl max-w-lg w-full text-center border border-gray-200 dark:border-gray-700">
              <div className="w-20 h-20 bg-green-100 dark:bg-green-900/30 text-green-500 rounded-full flex items-center justify-center mx-auto mb-6"><Save size={40} /></div>
              <h2 className="text-2xl font-bold font-brand text-gray-800 dark:text-white mb-2">Consultation Terminée</h2>
              <p className="text-gray-600 dark:text-gray-400 mb-8">Les informations ont été enregistrées avec succès. Le patient peut maintenant régler les frais ou récupérer ses documents.</p>

              <div className="space-y-3 mb-8">
                {prescriptions.length > 0 && (
                  <button onClick={() => handleDownloadPdf("prescription")} className="w-full flex items-center justify-center gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 font-bold rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors border border-blue-200 dark:border-blue-800">
                    <Download size={20} /> Imprimer l'Ordonnance
                  </button>
                )}
                {exams.length > 0 && (
                  <button onClick={() => handleDownloadPdf("exam-request")} className="w-full flex items-center justify-center gap-2 p-3 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 font-bold rounded-xl hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors border border-purple-200 dark:border-purple-800">
                    <Download size={20} /> Imprimer la demande d'Examens
                  </button>
                )}
              </div>

              <button onClick={() => onClose(true)} className="px-8 py-3 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white font-bold rounded-xl hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors w-full">
                Fermer
              </button>
            </div>
          </div>
      ...
      {/* MODALES ENFANTS */}
      {/* ================================================================= */}
      <AddMedicationModal AutoRefreshPage={handleAutoRefreshPage} isOpen={isAddMedModalOpen} onClose={() => setIsAddMedModalOpen(false)} onAdd={(med) => setPrescriptions((prev) => [...prev, med])} availableArticles={allArticles} />
      ...
      {previewImageUrl && (<ImagePreviewModal imageSrc={previewImageUrl} isOpen={!!previewImageUrl} onClose={() => setPreviewImageUrl(null)} />)}
    </div>
  );
};