      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Select
            ...
            value={
              categoryOptions.find(
                (opt) => opt.value === payload.category_id,
              ) || null
            }
            onChange={(selected) =>
              setPayload({
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setPayload({ ...payload, barcode: e.target.value })
            }
          />
        </div>
        ...
        <div className="md:col-span-2 p-3 bg-slate-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md flex items-start gap-3">
          <div className="pt-0.5">
            <input
              ...
              onChange={(e) =>
                setPayload({ ...payload, track_batches: e.target.checked })
              }
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Laissez coché pour les médicaments. Décochez pour le matériel
              générique (biberons, brosses...) qui ne nécessite pas de suivi
              strict. Un lot "STANDARD" infini sera créé.
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-2 p-3 bg-slate-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md flex items-start gap-3">
          <div className="pt-0.5">
            <input
              ...
              onChange={(e) =>
                setPayload({ ...payload, is_prescripted: e.target.checked })
              }
              className="w-5 h-5 text-[#00a896] bg-white border-gray-300 rounded focus:ring-[#00a896] dark:bg-gray-700 dark:border-gray-600"
            />
          </div>
          <div>
            <label
              htmlFor="is_prescripted"
              className="text-sm font-bold text-slate-800 dark:text-white cursor-pointer block"
            >
              Soumis à prescription médicale (Ordonnance obligatoire)
            </label>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Cochez cette case si ce produit exige une ordonnance. Une alerte
              sera affichée pour le pharmacien lors du passage en caisse.
            </p>
          </div>
        </div>
      </div>
