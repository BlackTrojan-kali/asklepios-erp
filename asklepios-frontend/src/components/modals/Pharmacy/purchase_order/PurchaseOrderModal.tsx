...
// --- COMPOSANT DE SAISIE DE NOMBRE AVEC SECTEURS +/- ET PRÉVENTION DE SIGNES NÉGATIFS ---
...
const StepperInput: React.FC<StepperInputProps> = ({
  ...
  const handleIncrement = () => {
    const current = typeof numericVal === "number" ? numericVal : 0;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Interdire les signes - et la notation scientifique e/E
  ...
  return (
    <div
      className={`flex items-center rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 overflow-hidden ${className}`}
    >
      <button
        type="button"
        onClick={handleDecrement}
        disabled={typeof numericVal === "number" && numericVal <= min}
        className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-300 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-xs font-black shrink-0 select-none cursor-pointer"
        title="Diminuer"
      >
        -
      </button>
      <input
        ...
        className="w-full text-center px-1 py-1 text-sm font-bold bg-transparent outline-none text-slate-800 dark:text-white [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none min-w-0"
      />
      <button
        type="button"
        onClick={handleIncrement}
        className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-300 transition-colors text-xs font-black shrink-0 select-none cursor-pointer"
        title="Augmenter"
      >
        +
      </button>
...
export const PurchaseOrderModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    if (isOpen && order) {
      ...
      setDestinationPharmacyId(order.destination_pharmacy_id || "");
      if (order.lines) {
        setLines(
          order.lines.map((l) => ({
  ...
  // --- STYLES REACT-SELECT POUR LE DARK MODE ---
        ...
        backgroundColor: state.isSelected
          ...
          : state.isFocused
            ? isDark
              ...
              : "#f1f5f9"
            : "transparent",
        color: state.isSelected ? "#ffffff" : isDark ? "#f9fafb" : "#1e293b",
        cursor: "pointer",
      }),
  ...
  const filteredArticles = useMemo(() => {
    return articles.filter((article) => {
      ...
      const isLowStock = article.global_min_qty
        ? (article.stock_qty || 0) <= article.global_min_qty
        : false;
  ...
  const toggleArticleSelection = (articleId: number) => {
    ...
    if (exists) {
      ...
      setLines([
        ...lines,
        { article_id: articleId, qty_ordered: 1, unit_cost: 0 },
      ]);
    }
  };
  ...
  const applyBulkQuantity = () => {
    ...
    setBulkQty("");
    toast.success(
      `Quantité ${bulkQty} appliquée à ${lines.length} article(s).`,
    );
  };
  ...
  const branchOptions = useMemo(() => {
    return (branches || []).map((b) => ({
      value: b.id,
      label: `${b.name} (${b.type === "central_warehouse" ? "Dépôt Central" : "Point de Vente"})`,
    }));
  }, [branches]);

  const updateLineField = (
    articleId: number | "",
    field: keyof PurchaseOrderLinePayload,
    value: any,
  ) => {
    setLines(
      lines.map((l) =>
        l.article_id === articleId ? { ...l, [field]: value } : l,
      ),
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isAdmin && !destinationPharmacyId) {
      toast.error("Veuillez sélectionner la pharmacie destinataire.");
      return;
    }
    ...
    const hasInvalidLine = cleanLines.some(
      (l) => !l.article_id || l.qty_ordered <= 0,
    );
    if (hasInvalidLine) {
      toast.error(
        "Vérifiez les quantités : elles doivent être supérieures à 0.",
      );
      return;
    }
    ...
    const payload: PurchaseOrderPayload = {
      ...
      ...(isAdmin && destinationPharmacyId
        ? { destination_pharmacy_id: destinationPharmacyId }
        : {}),
      lines: cleanLines,
    };
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Sélectionnez vos articles à gauche et ajustez les détails à
                droite.
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
          ...
          <div className="w-full lg:w-5/12 bg-slate-50/50 dark:bg-gray-900/30 border-r border-gray-200 dark:border-gray-800 flex flex-col shrink-0">
                    ...
                    <div
                      ...
                      className={`p-3 rounded-xl border cursor-pointer transition-all flex items-center gap-3 ${
                        isSelected
                          ...
                          : "bg-white border-gray-200 hover:border-teal-300 dark:bg-gray-800 dark:border-gray-700"
                      }`}
                    >
                      <div
            ...
            <div className="p-4 border-t border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800/50 shrink-0">
              ...
              <div className="flex gap-2 items-center">
                <StepperInput
                  value={bulkQty === "" ? "" : Number(bulkQty)}
                  onChange={(val) =>
                    setBulkQty(val === "" ? "" : val.toString())
                  }
                  min={1}
                  step={1}
                  placeholder="50"
                ...
                <button
                  ...
                  className="flex-1 px-3 py-2 bg-slate-800 text-white hover:bg-slate-700 dark:bg-gray-700 dark:hover:bg-gray-600 rounded-lg text-sm font-bold transition-colors cursor-pointer"
                >
                  Appliquer à tous
                </button>
              </div>
          ...
          <div className="w-full lg:w-7/12 flex flex-col bg-white dark:bg-gray-900 shrink-0">
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
              {/* PHARMACIE DESTINATAIRE (ADMIN) & FOURNISSEUR */}
              <div className="bg-slate-50 dark:bg-gray-800/40 p-5 rounded-xl border border-gray-100 dark:border-gray-800 space-y-4">
                {isAdmin && (
                  <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-gray-300 mb-2 flex items-center gap-1.5">
                    ...
                    <Select
                      ...
                      value={
                        branchOptions.find(
                          (opt) => opt.value === destinationPharmacyId,
                        ) || null
                      }
                      onChange={(selected) =>
                        setDestinationPharmacyId(selected ? selected.value : "")
                      }
                      placeholder="Sélectionner la pharmacie à livrer..."
                      isClearable
                      styles={selectStyles}
                ...
                <div>
                  ...
                  <Select
                    ...
                    value={
                      providerOptions.find((opt) => opt.value === providerId) ||
                      null
                    }
                    onChange={(selected) =>
                      setProviderId(selected ? selected.value : "")
                    }
                    placeholder="Sélectionner le grossiste/fournisseur..."
                    isClearable
                    styles={selectStyles}
                    menuPortalTarget={document.body}
                    className="text-sm react-select-container"
                ...
                {lines.length === 0 ? (
                  <div className="p-8 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-xl text-center">
                    <p className="text-gray-400 dark:text-gray-500 text-sm">
                      Veuillez sélectionner des articles dans le catalogue à
                      gauche.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {lines.map((line) => {
                      ...
                      return (
                        <div
                          key={line.article_id}
                          className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 border border-gray-100 dark:border-gray-800 rounded-xl bg-slate-50/50 dark:bg-gray-800/20 hover:border-teal-200 transition-colors group"
                        >
                          {/* Nom & Suppression */}
                          <div className="flex-1 min-w-0">
                          ...
                          {/* Inputs de quantité et prix individuels avec Steppers */}
                          <div className="flex flex-wrap items-center gap-3 shrink-0">
                            <div className="w-32">
                              <label className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-semibold block mb-1">
                                Qté demandée
                              </label>
                              <StepperInput
                            ...
                            <div className="w-24 text-right">
                              ...
                              <span className="font-mono font-bold text-slate-700 dark:text-gray-300 text-sm">
                                {totalLine.toLocaleString("fr-FR")}
                              </span>
                            </div>
                          </div>
                        </div>
            ...
            <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-slate-50 dark:bg-gray-900/80 shrink-0 flex flex-col sm:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] text-slate-500 dark:text-gray-400 uppercase font-bold tracking-wider">
                    Montant Estimé
                  </p>
              ...
              <button
                ...
                className="w-full sm:w-auto px-8 py-3 text-sm font-bold bg-[#00a896] hover:bg-[#008f7e] text-white rounded-xl transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:hover:shadow-none cursor-pointer"
              >
                {actionLoading
                  ? "Enregistrement..."
                  : order
