...
// Fonction d'aide pour formater de manière très robuste l'emplacement avec le code entre parenthèses
const formatLocationLabel = (locationObj) => {
  if (!locationObj) return "Non rangé";
  if (typeof locationObj === "string") return locationObj;

  const aisle = locationObj.aisle || "";
  const shelf = locationObj.shelf || "";
  const codeSuffix = locationObj.code ? ` (${locationObj.code})` : "";

  if (aisle && shelf) return `${aisle}-${shelf}${codeSuffix}`;
  if (aisle) return `Allée ${aisle}${codeSuffix} `;
  if (shelf) return `Étagère ${shelf}${codeSuffix} `;
  if (locationObj.description) return `${locationObj.description}${codeSuffix}`;
  if (locationObj.name) return `${locationObj.name}${codeSuffix}`;
  if (locationObj.code) return `(${locationObj.code})`;
...
export const InventoryModal: React.FC<Props> = ({
  ...
  const { createInventory, updateInventory, actionLoading } =
    useInventoryStore();
  ...
  useEffect(() => {
    ...
    if (existingInventory) {
      // MODE ÉDITION
      const safeDate = existingInventory.execution_date
        ? String(existingInventory.execution_date).substring(0, 10)
        : new Date().toISOString().substring(0, 10);
      setExecutionDate(safeDate);
      setComment(existingInventory.comment || "");

      const loadedLines: LocalLine[] =
        existingInventory.lines?.map((l: any) => {
          const locObj =
            l.storage_location ||
            l.storageLocation ||
            l.batch?.storage_location ||
            l.batch?.storageLocation;

          return {
            batch_id: Number(l.batch_id),
            storage_location_id: l.storage_location_id
              ? Number(l.storage_location_id)
              : null,
            article_name: l.batch?.article?.name || "Article inconnu",
            batch_number: l.batch?.batch_number || "N/A",
            location_label: formatLocationLabel(locObj),
            system_qty: Number(l.system_qty),
            physical_qty:
              l.physical_qty === null || l.physical_qty === undefined
                ? ""
      ...
      const initialLines: LocalLine[] = stocks.map((s: any) => {
        const locObj =
          s.storage_location ||
          s.storageLocation ||
          s.batch?.storage_location ||
          s.batch?.storageLocation;

        return {
          batch_id: Number(s.batch_id),
          storage_location_id: s.storage_location_id
            ? Number(s.storage_location_id)
            : null,
  ...
  }, [isOpen, existingInventory?.id, stocks]);
  ...
  const handleQtyChange = (
    ...
    setLines((prev) =>
      prev.map((l) =>
        Number(l.batch_id) === Number(batchId) &&
        String(l.storage_location_id) === String(locId)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const uncounted = lines.filter(
      (l) =>
        l.physical_qty === "" ||
        l.physical_qty === null ||
        l.physical_qty === undefined,
    );
    if (uncounted.length > 0) {
      toast.error(
        `Il reste ${uncounted.length} ligne(s) non comptée(s). Saisissez 0 si le produit n'est plus là.`,
      );
      return;
    }
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-6xl h-[90vh] flex flex-col shadow-2xl border border-transparent dark:border-gray-800 animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 dark:text-white leading-tight">
                {existingInventory
                  ? `Modifier le brouillon N°${existingInventory.id}`
                  : "Nouvelle Fiche d'Inventaire"}
              </h2>
              <p className="text-xs text-indigo-600 dark:text-indigo-400">
                Renseignez les quantités physiquement présentes en rayon.
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors cursor-pointer"
          >
            <X size={20} />
          </button>
        </div>
            ...
            <div className="md:col-span-4 flex flex-col justify-end">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Chercher un article, un lot, un emplacement..."
          ...
          <div className="px-5 py-3 bg-slate-50/80 dark:bg-gray-900/80 border-b border-gray-100 dark:border-gray-800 shrink-0 flex justify-between items-center">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-mono bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 px-2 py-0.5 rounded text-indigo-600 dark:text-indigo-400 font-bold">
                {lines.filter((l) => l.physical_qty !== "").length} /{" "}
                {lines.length}
              </span>
            </div>
            <button
              ...
              className="flex items-center gap-2 text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 dark:text-indigo-400 py-1.5 px-3 rounded-lg transition-colors border border-indigo-200 dark:border-indigo-800/50 cursor-pointer"
            >
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5">
            {stockLoading && lines.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-500">
                <AlertCircle
                  size={32}
                  className="animate-pulse mb-3 text-indigo-400"
                />
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-100 dark:bg-gray-800 sticky top-0 z-10 shadow-sm border-b border-gray-200 dark:border-gray-700">
                    ...
                    <th className="p-3 text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase text-center bg-indigo-50/50 dark:bg-indigo-900/20">
                      Qté Physique
                    </th>
                    <th className="p-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase text-center">
                      Écart
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {filteredLines.map((line) => {
                    ...
                    const discrepancy = isCounted
                      ? Number(line.physical_qty) - line.system_qty
                      : 0;

                    return (
                      <tr
                        key={`${line.batch_id}-${line.storage_location_id}`}
                      ...
                      >
                        ...
                        <td className="p-3 text-center text-xs text-gray-500 dark:text-gray-400">
                          <span
                            className={`inline-block px-2 py-0.5 rounded-md font-semibold ${
                              line.location_label !== "Non rangé"
                                ? "bg-indigo-50 text-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/30"
                        ...
                        {/* CHAMP DE SAISIE AVEC BOUTONS + ET - ET SANS NATIVE SPINBAR */}
                        <td className="p-2 text-center bg-indigo-50/20 dark:bg-indigo-900/10">
                          <div className="inline-flex items-center justify-center bg-white dark:bg-gray-800 p-1 rounded-xl border border-indigo-200/80 dark:border-gray-700 shadow-2xs group hover:border-indigo-400 dark:hover:border-indigo-500 transition-all">
                            <button
                              type="button"
                              onClick={() => {
                                const current =
                              ...
                              className="w-7 h-7 flex items-center justify-center rounded-lg bg-slate-100 hover:bg-indigo-600 hover:text-white dark:bg-gray-700 dark:hover:bg-indigo-600 text-slate-600 dark:text-gray-200 font-bold transition-all cursor-pointer select-none active:scale-95 shadow-2xs"
                              title="Diminuer"
                            >
                            ...
                            <input
                              ...
                              className={`w-16 text-center font-mono font-bold text-sm bg-transparent outline-none border-none focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
                                line.physical_qty === ""
                                  ? "text-red-500 placeholder-red-300 dark:placeholder-red-700"
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-slate-50 dark:bg-gray-900/80 shrink-0 flex justify-end items-center gap-3 rounded-b-2xl">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-800 rounded-xl transition-colors cursor-pointer"
          >
            Annuler
          </button>
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 text-sm font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-all shadow-md disabled:opacity-50 cursor-pointer"
          >
            <Save size={16} />
            {actionLoading
              ? "Enregistrement..."
