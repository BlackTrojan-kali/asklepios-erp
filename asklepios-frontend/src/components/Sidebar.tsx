...
const Sidebar = () => {
  ...
  // Extraction des licences autorisées
  ...
  // 👉 NOUVEAU : Déterminer si l'utilisateur est un Admin Global (Sans restriction)
  const isGlobalAdmin = useMemo(() => {
    if (userRole === "super_admin" || userRole === "ceo") return true;
    
    if (userRole === "admin") {
      const pAdmin = (profile as any)?.profile_admin;
      if (!pAdmin) return false;
      
      // Si un de ces tableaux contient au moins un ID, l'admin a un accès restreint (il n'est pas global)
      const isRestricted = 
        (pAdmin.center_ids && pAdmin.center_ids.length > 0) ||
        (pAdmin.pharmacy_branch_ids && pAdmin.pharmacy_branch_ids.length > 0) ||
        (pAdmin.laboratory_ids && pAdmin.laboratory_ids.length > 0) ||
        (pAdmin.accessible_licences && pAdmin.accessible_licences.length > 0);
        
      return !isRestricted; // S'il n'est pas restreint, il est global.
    }
  ...
  const filteredMenu = useMemo(() => {
    ...
    return MENU_CONFIG.filter((item) => {
      ...
      // 👉 2. NOUVEAU : VÉRIFICATION ADMIN GLOBAL
      if (item.globalAdminOnly && !isGlobalAdmin) {
        return false; // On cache le menu pour les admins restreints
      }

      // 3. VÉRIFICATION DE LA LICENCE (Si le menu exige une licence et qu'on n'est pas super_admin)
      ...
      // 4. EXCLUSION DE LICENCE (Si le menu est interdit quand une certaine licence est présente)
      ...
      // 5. S'il s'agit du rôle pharmacie ET que le menu cible une position spécifique
      if (
        userRole === "pharmacy" &&
        item.positions &&
      ...
      // 6. S'il s'agit du rôle laboratory ET que le menu cible un labRole spécifique
      if (
        userRole === "laboratory" &&
        item.labRoles &&
  ...
  }, [userRole, userPosition, allowedLicences, isGlobalAdmin]);
